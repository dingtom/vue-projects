  ...
  <div>
    ...
    <el-card>
      ...
      <tree-table class="treeTable" :data="catelist" :columns="columns" :selection-type="false" :expand-type="false"
        ...
        <!--expand-type	是否为展开行类型表格 selection-type	是否为多选类型表格 index-text序列标题 show-row-hover	鼠标悬停时，是否高亮当前行-->
        <!-- 是否有效 -->
...
export default {
  data() {
    return {
      ...
      // 将要添加的分类的名称// 父级分类的Id // 分类的等级，默认要添加的是1级分类
      // 添加分类表单的验证规则对象
      addCateFormRules: {
  ...
  methods: {
    ...
    async getCateList() {
      ...
      console.log(' getCateList', res.data)
      // 把数据列表，赋值给 catelist
    ...
    // 选择项发生变化触发这个函数
    parentCateChanged() {
      console.log('parentCateChanged', this.selectedKeys)
